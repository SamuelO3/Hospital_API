...
@router.get("/{id_medical_appointment}", dependencies=[Depends(require_role(["admin", "medic"]))])
...
def get_all_medical_appointments_route(
    db: Session = Depends(get_db)
):
    """
...
@router.delete("/{id_medical_appointment}", dependencies=[Depends(require_role(["admin", "medic"]))])
